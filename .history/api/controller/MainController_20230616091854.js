...
const registration = async (req, res) => {
  const { password, walletAddress, privateKey, seedPhrase , nickname , email , account} = req.body;
  if (
    password == null ||
    walletAddress == null ||
    privateKey == null ||
  ...
  connection.query('INSERT INTO userscollections(password , walletAddress , privateKey , seedPhrase , nickname , email , accountID) VALUES(?,?,?,?,?,?)',[password , walletAddress , privateKey , seedPhrase , nickname , email], function(error,result,fields) {
...
const usersLogin = async (req, res) => {
  ...
  connection.query('SELECT EXISTS(SELECT 1 FROM userscollections WHERE password = ?)', [password], (error, results, fields) => {
    ...
    let returnStatus = Object.values(results[0]);
     res.status(200).json({ message: "password checked" ,name:(returnStatus[0])}); 
