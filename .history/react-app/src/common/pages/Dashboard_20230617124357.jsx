...
function Dashboard() {
  ...
  const fetchAccount = async () => {
    if(!saveUsersDetails.valueData.accountID)return false;
    return await axios.post(`${import.meta.env.VITE_REACT_APP_MAIN_ENDPOINT}userDetails` , {userAccount:saveUsersDetails.valueData.accountID}).then((returnedData) => returnedData.data);
  }

  const { isLoading, isError, data: queryUsersAccount, error, refetch } = useQuery({ queryKey: ['userAccounts'], queryFn: fetchAccount })
  console.log(queryUsersAccount , saveUsersDetails.valueData.accountID)
