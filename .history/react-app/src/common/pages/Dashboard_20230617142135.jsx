...
const usersData = queryUsersAccount?.userAccountInfo;
  return (
      <MainLayout className="">
        { isLoading ?"loading..." :
          (<div>
          <div className='border min-h-[50px] border-orange-300 justify-evenly items-center w-11/12 lg:w-10/12  rounded-full my-3 flex flex-row px-3'>     
            <div className="text-xs flex">{"Account Id:  "} <span className="font-semibold text-sm">{usersData.accountId}</span></div>
            <div className="text-xs">{"Balance: "} <span className="font-semibold text-sm">{usersData.balance}</span></div>
         
          </div>
          <div className='border min-h-[50px] border-orange-300 justify-between items-center w-11/12 lg:w-9/12  rounded-full my-3 flex flex-row px-3'>     
