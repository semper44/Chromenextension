...
const usersData = queryUsersAccount?.userAccountInfo;
  return (
      <MainLayout className="">
        { isLoading ?"loading..." :
          (<div>
          <div className='border min-h-[50px] border-orange-300 justify-center space-y-3 lg:justify-evenly items-center w-11/12 lg:w-11/12  rounded-full my-3 flex flex-row px-3'>     
            ...
            <div className="text-xs w-fit text-center ">{"Balances: "} <span className="font-semibold text-sm w-fit">{usersData.balance}</span></div>
            <div className="text-xs w-fit text-center capitalize ">{"ledgerId: "} <span className="font-semibold text-sm w-fit">{usersData.ledgerId}</span></div>
          </div>
          ...
          <div className='w-full'>
              <table border>
                <thead>
                  <tr className=''>
                  <th rowSpan={2} className='capitalize  px-10'>proxyAccountId</th>
                  <th className='capitalize  px-10'>proxyReceived</th>
                  </tr>
                </thead>
                 <tbody>
                    <tr>
                      <td className='text-center w-full px-10'>{(usersData.proxyAccountId == null)? "null" : usersData.proxyAccountId}</td>
                      <td className='text-center w-full px-10'>{(usersData.proxyAccountId == null)? "null" : usersData.proxyReceived}</td>
