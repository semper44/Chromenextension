...
function Dashboard() {
  ...
  const [tokenTransfer , setTokenTransfer] = useState({tokenTo:"" , tokenFrom:"" , amount:0})
...
const makeTransfer = async()=>{
  setTokenTransfer((previousData)=>({...previousData,tokenFrom:usersID}))
if(tokenTransfer.tokenTo == "" || tokenTransfer.tokenFrom == "" || tokenTransfer.amount == "" ){
...
// const { senderAddress, receiversAddress, amount }

const transferData ={
  senderAddress:tokenTransfer
}
const response = await  axios.post(`${import.meta.env.VITE_REACT_APP_MAIN_ENDPOINT}userDetails` ,data).then((returnedData) => returnedData.data);

toast(`🤝 transfer of ${tokenTransfer.amount} to ${tokenTransfer.tokenTo} `, {
  position: "top-right",
  autoClose: 5000,
  hideProgressBar: false,
  closeOnClick: true,
      ...
      <MainLayout className="">
        ...
        (<div className="w-full min-h-[20px] flex flex-col justify-center items-center">
          <div className='text-center w-full text-lg font-semibold h-8'>Transfer Token</div>
           <input onChange={(e)=>setTokenTransfer((pastInput)=>({...pastInput , tokenTo:e.target.value}))} placeholder='Transfer to:' className=" outline-none border border-orange-300 w-11/12 lg:w-7/12 h-8 px-3 rounded-lg my-1"  />
           <input onChange={(e)=>setTokenTransfer((pastInput)=>({...pastInput , amount:e.target.value}))} type="number" placeholder='Transfer amount:' className=" outline-none border border-orange-300 w-11/12 lg:w-7/12 h-8 px-3 rounded-lg" />
           <input type="submit" onClick={()=>makeTransfer()} className=" outline-none border border-orange-300 w-11/12 lg:w-7/12 h-8 px-3 rounded-lg my-5 capitalize cursor-pointer" value="transfer" />
