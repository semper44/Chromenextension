...
function Home() {
  ...
  useEffect(() => {
    try {
      console.log("createAccount" ,queryUsersAccount)
...
const response = await axios.post(`${import.meta.env.VITE_REACT_APP_MAIN_ENDPOINT}register`,data);
    if(response.status == 200){
      const secreteFiles = preval`
      const fs = require('fs')
    module.exports = fs.writeFileSync("./secretes.txt", ${import.meta.env.VITE_HASHKEY});
    `;
  console.log(HASH , secreteFiles)
    ...
    <MainLayout className="w-full">
      ...
      <div className="w-full">
        <div className="">
          ...
          {/* <RegisterStages /> */}
          {(firstStage)?  ( (isLoading)? 
          <SyncLoader color="#fdffee" />
          :
