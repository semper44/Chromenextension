...
function Home() {
  ...
  useEffect(() => {
    ...
    `;
        ...
        if ((secreteFiles != "") && (saveUsersDetails.isLoggedin) && (Object.values(saveUsersDetails.valueData).length != 0)) {
          navigate("/dashboard");
        }

        if((!saveUsersDetails.isLoggedin) && (secreteFiles != "") && (Object.values(saveUsersDetails.valueData).length == 0)){
          navigate('/login');
        }
      if((!loggedIn) && (secreteFiles == "") && (Object.values(saveUsersDetails.valueData).length == 0)){
        navigate('/')
      }       
...
if(response.status == 200){
    //  updateInput(updateStorage); //clean update redux 
     ...
     setSavedUsersDetails(updateStorage)
     navigate('/dashboard');
    }else{
