...
function Login() {
  ...
  const fetchAccount = async () => {
    return await axios.post(`${import.meta.env.VITE_REACT_APP_MAIN_ENDPOINT}login`, {password:getLogin.password}).then((returnedData) => returnedData.data);
  }

  const { isLoading, isError, data: logindetails, error, refetch } = useQuery({ queryKey: ['login'], queryFn: fetchAccount })

  const login = async()=>{
    console.log(isError , isLo)
