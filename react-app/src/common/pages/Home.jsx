...
function Home() {
  ...
  const submitAndNavigate = async () => {
    ...
    if (!isError && !isLoading) {
      ...
      setRegister((previousData) => ({
        ...previousData,
        usersAccount: queryUsersAccount.publicKeys,
        usersPrivateKey: queryUsersAccount.privateKeys,
  ...
  const navigateFouth = () => {
    if (matchingSeed[3] == "" && matchingSeed[5] == "" && matchingSeed[7] == "")
    ...
    if (
      seedPhrase[2] == matchingSeed[3] &&
      seedPhrase[4] == matchingSeed[5] &&
      seedPhrase[6] == matchingSeed[7]
    ) {
  ...
  const registerUsers = async () => {
    return await axios
      .get(`${import.meta.env.VITE_REACT_APP_MAIN_ENDPOINT}register`, {})
      .then((returnedData) => returnedData.data);
  };
  const {
    isLoading: loadings,
    isError: errorss,
    data: queryregistrations,
    refetch: registerRefreshs,
  } = useQuery({ queryKey: ["usersAccount"], queryFn: registerUsers });
  ...
  const navigateUsersPage = async () => {
    try {
      ...
      if (
        ...
        register.password !== "" &&
        register.nickname !== "" &&
        register.usersAccount !== "" &&
        register.usersPrivateKey !== "" &&
        register.usersSeedPhrase !== ""
      ) {
        let data = {
            ...
            <form className="w-full my-6 flex justify-center items-center  flex-col">
              <div className="w-full flex items-start justify-start ">
                <FiArrowLeft
                  color="orange"
                  size={20}
                  onClick={() => backPageValidateSecrete()}
                />
              </div>
              <div className="w-full">
                <InputsWithValidation
